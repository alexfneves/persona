...
3. **audio.cpp is built by ITS OWN flake** (input `audiocpp`) — never reimplement its cmake build. persona grafts library artifacts (static archives, headers, VAD assets) onto audio.cpp's package via `graftLibraryArtifacts` (`overrideAttrs` installPhase) in `flake.nix`. Model set is full (all 49 families), backend via their `vulkanSupport`. Other system libraries go into the `persona` derivation's `buildInputs` **and** its `clang++` link line.
